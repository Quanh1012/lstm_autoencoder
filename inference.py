...
def check_val(model_path, data_set, classification_model, mode):
    # data = {}
    # with np.load(data_path, 'rb') as f:
    #     data['input'] = f['data'].transpose(0,2,1)
    #     data['label'] = f['label']
    # data['input'] = np.expand_dims(data['input'], axis=1)
    # data['input'] = torch.Tensor(data['input'])
    ...
    with torch.no_grad():
       ...
       for i, data in enumerate(data_set):
                ...
                target = data['labels'].float().to(device)
...
#         print((bee_ + noise_ + cricket_)*100/3000)
    ...
    acc_test= check_val(config['best_model_path'], val_set_, classification_model, 'val')
    print(acc_test)
